...
(function(){
    ...
    addAdmSection('p-final', `
      ...
      <div class="v23-final-board">
        ...
        <div class="v23-fb-actions">
          <button class="v23-btn primary" onclick="batchApproveBoardAuto()">🚀 즉시승인 ${_autoFinal.length}건 일괄</button>
  ...
  window.refreshFinalApproval=function(){
    ...
    // 필터 값 보존 (팀/점수/유형/검색) — 재렌더 후 복원해 '팀 선택 상태'가 유지되게
    var fv={t:($('#fTeamFilt')||{}).value, s:($('#fScoreFilt')||{}).value, ty:($('#fTypeFilt')||{}).value, q:($('#fSearchFinal')||{}).value};
    if(typeof renderAdmin==='function') renderAdmin();
    var btn=document.querySelector('.atb[onclick*="p-final"]'); if(window.at) window.at(btn,'p-final');
    if($('#fTeamFilt')&&fv.t)$('#fTeamFilt').value=fv.t;
  ...
  window._updateFinalBoard=function(){
    var board=$('.v23-final-board'); if(!board) return;
    var team=($('#fTeamFilt')&&$('#fTeamFilt').value)||'';
    var pend=teamDocs.filter(function(d){return d.status==='등록 요청됨'&&(!team||d.team===team);});
    var done=teamDocs.filter(function(d){return d.status==='AI 검색 반영완료'&&(!team||d.team===team);}).length;
    var a=pend.filter(function(d){return _qScore(d)>=85;}).length;
    var r=pend.filter(function(d){var s=_qScore(d);return s>=70&&s<85;}).length;
    var l=pend.filter(function(d){return _qScore(d)<70;}).length;
    var tot=pend.length;
    var cntEl=board.querySelector('.v23-fb-count'); if(cntEl)cntEl.innerHTML=tot+'<span>건</span>';
    var lblEl=board.querySelector('.v23-fb-label'); if(lblEl)lblEl.textContent=team?(team+' 대기'):'검토 대기 문서';
    var doneEl=board.querySelector('.v23-fb-done'); if(doneEl)doneEl.textContent='✅ AI 반영 '+done+'건 · 진행률 '+((done+tot)>0?Math.round(done/(done+tot)*100):0)+'%';
    var vals=[a,r,l]; board.querySelectorAll('.v23-fb-row').forEach(function(row,i){
      var b=row.querySelector('b'); if(b)b.textContent=vals[i]+'건';
      var bar=row.querySelector('.v23-fb-track>span'); if(bar)bar.style.width=(tot?Math.round(vals[i]/tot*100):0)+'%';
    });
    var pb=board.querySelector('.v23-fb-actions .v23-btn.primary'); if(pb)pb.textContent='🚀 즉시승인 '+a+'건 일괄';
  };
  // 보드 '즉시승인 일괄' — 현재 팀 필터 범위만 승인 (선택한 팀 기준)
  window.batchApproveBoardAuto=function(){
    var team=($('#fTeamFilt')&&$('#fTeamFilt').value)||'';
    var ids=_finalsInLane('auto',team).map(function(d){return d.id;});
    if(!ids.length){safeToast('즉시 승인 대상이 없습니다.','⚠️');return;}
    var n=_approveDocs(ids);
    safeToast('✅ '+(team?team+' ':'')+n+'건 즉시 승인 완료 — AI DB 반영','✅',3200);
...
/* ═══════════════════════════════════════════════════════════════
 * §16  AI 에이전트 마켓 — 구매본부 지식 커뮤니티 신규 탭
 *      클로드 코드로 만든 업무 자동화 Agent(.exe)를
 *      5개 영역(구매·개발·품질·관리·구매공통)별로 공유/다운로드.
 *      ※ 커뮤니티 모달(#commOv)은 frozen React DOM →
 *        본 레이어가 탭 버튼/패널을 동적 주입하고 직접 관리한다.
 * ═══════════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  var $  = function(s,r){ return (r||document).querySelector(s); };
  ...
  var say = function(m,i,d){ try{ (window.toast||window.say||console.log)(m,i||'✅',d||2200); }catch(e){ console.log(i,m); } };

  /* 표준 커뮤니티 패널 (이 목록 외에 ct-agent를 추가 관리) */
  var STD_PANELS = ['ct-qa','ct-hof','ct-lv','ct-char','ct-shop','ct-quest'];
  ...
  function seedAgents(){
    return [
      { id:1,  area:'purchase', icon:'📢', name:'입찰공고 자동생성 Agent',   ver:'2.1', desc:'품의 정보를 입력하면 구매업무규정에 맞춘 입찰 공고문 초안을 자동 작성합니다. 탄력적입찰·수의계약 양식 지원.', author:'프로큐어마스터', lv:5, size:'18.4 MB', dl:342, updated:'2일 전', tags:['입찰','공고문','규정자동'] },
      { id:2,  area:'purchase', icon:'⚖️', name:'단가비교 분석 Agent',        ver:'1.4', desc:'협력사 견적서(엑셀/PDF)를 읽어 품목별 단가 비교표와 최저가·이상치를 자동 산출합니다.', author:'구매전략가', lv:4, size:'22.7 MB', dl:198, updated:'5일 전', tags:['견적','단가비교','협력사'] },
      { id:3,  area:'purchase', icon:'📝', name:'수의계약 사유서 작성 Agent',  ver:'1.0', desc:'긴급/독점 사유를 선택하면 결재 기준에 맞춘 수의계약 사유서를 생성합니다. 전결 금액 자동 판정.', author:'계약초보', lv:2, size:'12.1 MB', dl:87, updated:'어제', tags:['수의계약','사유서','결재'] },
      ...
      { id:9,  area:'quality', icon:'⭐', name:'5스타 평가 집계 Agent',      ver:'1.5', desc:'협력사 5스타 평가 항목 점수를 집계해 등급 판정과 승급/강등 대상을 리포트합니다.', author:'평가달인', lv:4, size:'14.3 MB', dl:143, updated:'3일 전', tags:['5스타','평가','등급'] },

      { id:10, area:'manage', icon:'📊', name:'월간 KPI 리포트 Agent',     ver:'2.0', desc:'구매혁신 KPI 데이터를 취합해 달성률·전월대비 차트가 포함된 월간 리포트를 자동 작성합니다.', author:'데이터마법사', lv:5, size:'26.5 MB', dl:419, updated:'어제', tags:['KPI','리포트','대시보드'] },
  ...
  var st = window.__agentMkt = window.__agentMkt || { list: seedAgents(), area:'all', q:'', upload:false, officialOnly:false };
  ...
  window.__agentCanApprove = canApprove;
  /* 마켓/패널/Admin 동시 새로고침 */
  function agentRefreshAll(){ try{ renderAgents(); }catch(e){} try{ window.rpRenderAgents && window.rpRenderAgents(); }catch(e){} try{ window.renderAdminAgents && window.renderAdminAgents(); }catch(e){} }
  window.agentRefreshAll = agentRefreshAll;

  /* 영역별 라인 아이콘 (촌스러운 이모지 대체) */
  function areaSvg(areaId, size){
    var ar=areaInfo(areaId), c=ar.c||'#4b8ef0', sz=size||22;
    var p={
      purchase:'<circle cx="9" cy="20" r="1.3"/><circle cx="18" cy="20" r="1.3"/><path d="M2 3h3l2.2 12.2a1.5 1.5 0 0 0 1.5 1.3h8.3a1.5 1.5 0 0 0 1.5-1.2L21.5 7H6"/>',
      dev:'<polyline points="8 8 4 12 8 16"/><polyline points="16 8 20 12 16 16"/><line x1="13.5" y1="6" x2="10.5" y2="18"/>',
      quality:'<path d="M12 3l7 3v5c0 4.5-3 7.6-7 9-4-1.4-7-4.5-7-9V6z"/><path d="M9 12l2 2 4-4"/>',
      manage:'<rect x="4" y="4" width="16" height="16" rx="2"/><line x1="8" y1="14" x2="8" y2="17"/><line x1="12" y1="10" x2="12" y2="17"/><line x1="16" y1="12" x2="16" y2="17"/>',
      common:'<circle cx="12" cy="12" r="2.1"/><circle cx="5" cy="6" r="1.7"/><circle cx="19" cy="6" r="1.7"/><circle cx="5" cy="18" r="1.7"/><circle cx="19" cy="18" r="1.7"/><path d="M10.4 10.7 6.7 7.6M13.6 10.7l3.7-3.1M10.4 13.3 6.7 16.4M13.6 13.3l3.7 3.1"/>'
    };
    var body = p[areaId] || '<rect x="5" y="8" width="14" height="10" rx="2.5"/><line x1="12" y1="5" x2="12" y2="8"/><circle cx="12" cy="4" r="1"/><circle cx="9.5" cy="13" r="1"/><circle cx="14.5" cy="13" r="1"/>';
    return '<svg width="'+sz+'" height="'+sz+'" viewBox="0 0 24 24" fill="none" stroke="'+c+'" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">'+body+'</svg>';
  }
  function statusBadgeHtml(s){ var si=statusInfo(s); return '<span class="agent-status" style="background:'+si.c+'1f;color:'+si.c+'">'+si.icon+' '+si.label+'</span>'; }
  window.agentAreaSvg = areaSvg;
  window.agentStatusBadge = statusBadgeHtml;

  /* ── 스타일 주입 (1회) ── */
  function injectStyle(){
    if(document.getElementById('agentMktStyle')) return;
    var css = [
      '#ct-agent .agent-hd{display:flex;gap:12px;align-items:flex-start;margin-bottom:14px;flex-wrap:wrap}',
      '#ct-agent .agent-hd-txt{flex:1;min-width:220px}',
      '#ct-agent .agent-title{font-size:15px;font-weight:800;margin-bottom:4px;display:flex;align-items:center;gap:6px}',
      '#ct-agent .agent-sub{font-size:10.5px;color:var(--text-3);line-height:1.6;background:var(--bg-2);padding:8px 12px;border-radius:8px;border:1px solid var(--border-1)}',
      '#ct-agent .agent-share-btn{flex-shrink:0;font-size:12px;font-weight:700;padding:9px 16px;background:var(--accent);color:#fff;border:none;border-radius:9px;cursor:pointer;font-family:inherit;box-shadow:0 2px 8px rgba(75,142,240,.32);transition:.15s;white-space:nowrap}',
      '#ct-agent .agent-share-btn:hover{filter:brightness(1.08);transform:translateY(-1px)}',
      '#ct-agent .agent-toolbar{display:flex;gap:8px;align-items:center;margin-bottom:12px;flex-wrap:wrap}',
      '#ct-agent .agent-search{flex:1;min-width:160px;display:flex;align-items:center;gap:6px;background:var(--bg-3);border:1px solid var(--border-1);border-radius:8px;padding:7px 11px}',
      '#ct-agent .agent-search input{flex:1;background:none;border:none;outline:none;color:var(--text-1);font-size:12px;font-family:inherit}',
      '#ct-agent .agent-chips{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px}',
      '#ct-agent .agent-chip{font-size:11px;font-weight:600;padding:6px 12px;border-radius:999px;border:1.5px solid var(--border-2);background:var(--bg-2);color:var(--text-2);cursor:pointer;transition:.12s;font-family:inherit;display:flex;align-items:center;gap:4px}',
      ...
      '#ct-agent .agent-tags{display:flex;gap:4px;flex-wrap:wrap}',
      '#ct-agent .agent-tag{font-size:9px;color:var(--text-4);background:var(--bg-4,var(--bg-2));border:1px solid var(--border-1);border-radius:5px;padding:1px 6px}',
      '#ct-agent .agent-meta{display:flex;gap:9px;flex-wrap:wrap;font-size:9.5px;color:var(--text-4);border-top:1px solid var(--border-1);padding-top:8px}',
      '#ct-agent .agent-acts{display:flex;gap:6px}',
      '#ct-agent .agent-dl-btn{flex:1;font-size:11.5px;font-weight:700;padding:8px;background:var(--accent);color:#fff;border:none;border-radius:8px;cursor:pointer;font-family:inherit;transition:.13s}',
      '#ct-agent .agent-dl-btn:hover{filter:brightness(1.08)}',
      '#ct-agent .agent-info-btn{flex-shrink:0;width:34px;font-size:13px;background:var(--bg-3);border:1px solid var(--border-2);border-radius:8px;color:var(--text-2);cursor:pointer}',
      '#ct-agent .agent-info-btn:hover{border-color:var(--accent);color:var(--accent)}',
      '#ct-agent .agent-empty{text-align:center;padding:40px 14px;color:var(--text-4);font-size:12px}',
      '#ct-agent .agent-upload{background:var(--bg-2);border:1px dashed var(--border-2);border-radius:12px;padding:16px;margin-bottom:14px}',
      '#ct-agent .agent-upload h4{font-size:12.5px;font-weight:800;margin-bottom:10px;display:flex;align-items:center;gap:6px}',
      '#ct-agent .agent-fld{margin-bottom:9px}',
      '#ct-agent .agent-fld label{display:block;font-size:10px;font-weight:700;color:var(--text-3);margin-bottom:4px}',
      '#ct-agent .agent-fld input,#ct-agent .agent-fld select,#ct-agent .agent-fld textarea{width:100%;background:var(--bg-3);border:1px solid var(--border-1);border-radius:7px;padding:8px 10px;color:var(--text-1);font-size:12px;font-family:inherit;outline:none}',
      '#ct-agent .agent-fld textarea{resize:vertical;min-height:54px;line-height:1.6}',
      '#ct-agent .agent-up-acts{display:flex;justify-content:flex-end;gap:8px;margin-top:4px}',
      '#ct-agent .agent-up-acts .ok{font-size:12px;font-weight:700;padding:8px 16px;background:var(--accent);color:#fff;border:none;border-radius:8px;cursor:pointer;font-family:inherit}',
      '#ct-agent .agent-up-acts .cancel{font-size:12px;padding:8px 14px;background:var(--bg-3);border:1px solid var(--border-2);border-radius:8px;color:var(--text-2);cursor:pointer;font-family:inherit}',
      '#ct-agent .agent-ic svg{display:block}',
      '#ct-agent .agent-status{font-size:9px;font-weight:800;padding:2px 8px;border-radius:999px;white-space:nowrap}',
      '#ct-agent .agent-card.is-review{opacity:.94}',
      '#ct-agent .agent-card.is-review::before{opacity:.5}',
      '#ct-agent .agent-dl-btn.pending{background:var(--bg-3);color:var(--text-3);border:1px solid var(--border-2);font-weight:600}',
      '#ct-agent .agent-dl-btn.pending:hover{filter:none;border-color:var(--accent);color:var(--accent)}',
      '#ct-agent .agent-promote{flex-shrink:0;font-size:11px;font-weight:700;padding:8px 10px;background:rgba(43,182,115,.16);color:#2bb673;border:1px solid rgba(43,182,115,.4);border-radius:8px;cursor:pointer;font-family:inherit;white-space:nowrap}',
      '#ct-agent .agent-promote:hover{background:#2bb673;color:#fff}',
      '#ct-agent .agent-official-toggle{margin-left:auto}',
  ...
  /* ── 탭 버튼 + 패널 주입 (idempotent) ── */
  function ensureAgentTab(){
    var tabs = $('.comm-tabs'), body = $('.comm-body');
    if(!tabs || !body) return false;
    injectStyle();
    if(!$('#agentTabBtn')){
      var b = document.createElement('button');
      b.className='comm-tab'; b.id='agentTabBtn';
      b.textContent='🤖 AI 에이전트';
      b.setAttribute('onclick','agentTab(this)');
      tabs.appendChild(b);
    }
    if(!$('#ct-agent')){
  ...
  function filtered(){
    ...
    var list = st.list.filter(function(a){
      if(st.area!=='all' && a.area!==st.area) return false;
      if(st.officialOnly && a.status!=='official') return false;
      if(q){
        var hay = (a.name+' '+a.desc+' '+(a.tags||[]).join(' ')+' '+a.author).toLowerCase();
        if(hay.indexOf(q)===-1) return false;
      }
      return true;
    });
    var rank={official:0,poc:1,review:2};
    list.sort(function(a,b){ return (rank[a.status]==null?1:rank[a.status])-(rank[b.status]==null?1:rank[b.status]) || (b.dl||0)-(a.dl||0); });
    return list;
  }
  ...
  function uploadHtml(){
    if(!st.upload) return '';
    var ed = st.editId ? (function(){ for(var i=0;i<st.list.length;i++){ if(st.list[i].id===st.editId) return st.list[i]; } return null; })() : null;
    var opts = AREAS.map(function(ar){ return '<option value="'+ar.id+'"'+(ed&&ed.area===ar.id?' selected':'')+'>'+ar.icon+' '+ar.name+'</option>'; }).join('');
    return '<div class="agent-upload">'
      + '<h4>'+(ed?'✏️ 에이전트 수정':'➕ 에이전트 올리기')+'</h4>'
      + '<div class="agent-gate-txt">누구나 올리고 수정할 수 있어요. 올린 에이전트는 <b>🧪 임시(PoC)</b>로 등록되며, <b>ADMIN 최종 승인</b> 후 <b>✅ 공식</b>으로 전환됩니다.'+(ed&&ed.status==='official'?' <b style="color:#e0a13a">※ 공식 에이전트를 수정하면 다시 임시(PoC)로 내려가 재승인이 필요합니다.</b>':'')+'</div>'
      + '<div class="agent-fld"><label>에이전트 이름</label><input id="agName" value="'+(ed?esc(ed.name):'')+'" placeholder="예: 발주서 검증 Agent"></div>'
      + '<div class="agent-fld"><label>영역</label><select id="agArea">'+opts+'</select></div>'
      + '<div class="agent-fld"><label>설명</label><textarea id="agDesc" placeholder="어떤 업무를 자동화하나요? 입력/출력과 활용 팁을 적어주세요.">'+(ed?esc(ed.desc):'')+'</textarea></div>'
      + '<div class="agent-fld"><label>실행 파일 (.exe)'+(ed?' <span style="color:var(--text-4);font-weight:400">— 변경 시에만 첨부</span>':'')+'</label><input id="agFile" type="file" accept=".exe,.zip,.msi"></div>'
      + '<div class="agent-up-acts"><button class="cancel" onclick="agentToggleUpload(false)">취소</button><button class="ok" onclick="agentSubmit()">'+(ed?'수정 저장':'임시(PoC)로 올리기')+'</button></div>'
      + '</div>';
  }
  ...
  /* ── 탭 전환 ── */
  window.agentTab = function(btn){
    ensureAgentTab();
    STD_PANELS.forEach(function(id){ var el=document.getElementById(id); if(el) el.style.display='none'; });
    var ag = $('#ct-agent'); if(ag) ag.style.display='block';
    $$('.comm-tab').forEach(function(b){ b.classList.remove('on'); });
    if(btn) btn.classList.add('on'); else { var ab=$('#agentTabBtn'); if(ab) ab.classList.add('on'); }
    renderAgents();
  };

  /* ── 필터 / 검색 ── */
  window.agentFilter = function(area, btn){
    st.area = area;
    $$('#ct-agent .agent-chip').forEach(function(b){ b.classList.remove('on'); });
    if(btn) btn.classList.add('on');
    renderAgents();
  };
  window.agentSearch = function(v){ st.q = v||''; renderAgents(); };

  /* ── 업로드/수정 폼 토글 ── */
  window.agentToggleUpload = function(show){ st.upload = !!show; if(!show) st.editId=null; renderAgents(); if(show){ var n=$('#agName'); if(n) try{n.focus();}catch(e){} } };
  window.agentEdit = function(id){ st.editId=id; st.upload=true; renderAgents(); var n=$('#agName'); if(n) try{ n.focus(); n.setSelectionRange(n.value.length,n.value.length); }catch(e){} };
  function findAgent(id){ for(var i=0;i<st.list.length;i++){ if(st.list[i].id===id) return st.list[i]; } return null; }
  ...
  window.agentSubmit = function(){
    ...
    var size = (fileEl && fileEl.files && fileEl.files[0]) ? (fileEl.files[0].size/1048576).toFixed(1)+' MB' : null;
    if(st.editId){
      var a=findAgent(st.editId);
      if(a){ a.name=name; a.area=area; a.desc=desc||a.desc; if(size)a.size=size; a.updated='방금';
        a.status='temp'; /* 수정 시 재승인 필요 → 임시로 */ }
      st.upload=false; st.editId=null; st.area=area; agentRefreshAll();
      say('✏️ "'+name+'" 수정 완료 — 임시(PoC)로 전환되어 ADMIN 재승인이 필요합니다.','✅',3200);
      return;
    }
  ...
  /* ── 승인/반려/삭제 (ADMIN) ── */
  window.agentPromote = function(id){
    if(!canApprove()){ say('최종 승인은 ADMIN만 가능합니다.','🔒',2000); return; }
    var a=findAgent(id); if(!a) return; a.status='official'; a.updated='방금';
    agentRefreshAll();
    say('✅ "'+a.name+'" 최종 심사 통과 — 공식 에이전트로 승인했습니다.','✅',2800);
  };
  window.agentReject = function(id){
    if(!canApprove()){ say('승인 변경은 ADMIN만 가능합니다.','🔒',2000); return; }
    var a=findAgent(id); if(!a) return; a.status='temp';
    agentRefreshAll();
    say('↩️ "'+a.name+'"를 임시(PoC)로 되돌렸습니다.','↩️',2400);
  };
  window.agentDelete = function(id){
    var a=findAgent(id); if(!a) return;
    if(!confirm('"'+a.name+'" 에이전트를 삭제하시겠습니까?')) return;
    st.list = st.list.filter(function(x){ return x.id!==id; });
    agentRefreshAll();
  ...
  window.agentDownload = function(id){
    ...
    try{
      var manifest = [
        ...
        '',
        '[설치 안내]',
        '본 파일은 데모 환경의 매니페스트입니다.',
        '실제 배포 환경에서는 '+a.name+'.exe 실행 파일이 내려받아집니다.',
        '',
        '— 구매본부 지식 커뮤니티'
      ].join('\n');
      var blob = new Blob([manifest], { type:'text/plain;charset=utf-8' });
      var url = URL.createObjectURL(blob);
      var el = document.createElement('a');
      el.href = url;
      el.download = a.name.replace(/[\\/:*?"<>|\s]+/g,'_')+'_v'+a.ver+'_README.txt';
  ...
  window.agentDetail = function(id){
    ...
    say(si.icon+' '+si.label+' · '+a.name+' v'+a.ver+' · '+a.author+' · ⬇️'+(a.dl||0),'ℹ️',3600);
  };

  /* ── 표준 탭 전환 시 ct-agent 숨김 보장 (commTab/openComm 래핑) ── */
  function hideAgentPanel(){ var ag=$('#ct-agent'); if(ag) ag.style.display='none'; var ab=$('#agentTabBtn'); if(ab) ab.classList.remove('on'); }

  var _commTab = window.commTab;
  if(_commTab && !window.__agentCommTabWrapped){
    window.__agentCommTabWrapped = true;
    window.commTab = function(btn, id){
      if(_commTab) _commTab.apply(this, arguments);
      hideAgentPanel();
    };
  }
  var _openComm = window.openComm;
  if(_openComm && !window.__agentOpenCommWrapped){
    window.__agentOpenCommWrapped = true;
    window.openComm = function(tab){
      if(_openComm) _openComm.apply(this, arguments);
      setTimeout(function(){ ensureAgentTab(); if(tab!=='agent') hideAgentPanel(); }, 60);
    };
  }
...
(function(){
  ...
  var $  = function(s,r){ return (r||document).querySelector(s); };
  var esc= function(s){ return String(s==null?'':s).replace(/[&<>"']/g,function(m){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]; }); };
  var rpAg = { area:'all', q:'', officialOnly:false };
  ...
  function agentList(){
    ...
    var out=list.filter(function(a){
      ...
      if(rpAg.officialOnly && a.status!=='official') return false;
  ...
  function injectStyle(){
    ...
    var css=[
      ...
      '#rp .rpa-ic svg{display:block}',
      '#rp .agent-status{font-size:8.5px;font-weight:800;padding:1px 6px;border-radius:999px;white-space:nowrap;margin-right:3px}',
  ...
  function ensureReopen(){
    if(document.getElementById('rpReopen')) return;
    var b=document.createElement('button'); b.id='rpReopen'; b.title='작업 패널 열기';
    b.textContent='◀ 작업 패널';
    b.onclick=function(){ if(window.rpT) window.rpT(true); else { var rp=document.getElementById('rp'); if(rp) rp.classList.add('sh'); } syncReopen(); };
    document.body.appendChild(b);
  }
  function syncReopen(){ var rp=document.getElementById('rp'), h=document.getElementById('rpReopen'); if(!h) return; h.style.display=(rp && rp.classList.contains('sh'))?'none':'block'; }
  window.__rpSyncReopen = syncReopen;

  /* "가운데 창 크게(근거 크게 보기)" 제거 — 혼란 요소 */
  window.toggleSourceFull = function(){};
  window.toggleSourceFullV27 = function(){};
  function removeEnlarge(){
    var sf=document.getElementById('sourceFullBtn'); if(sf) sf.remove();
    var rp=document.getElementById('rp'); if(!rp) return;
    rp.classList.remove('source-full');
    Array.prototype.forEach.call(rp.querySelectorAll('button'), function(b){ var t=(b.textContent||'').trim(); if(t.indexOf('크게')>=0 && t.indexOf('⛶')>=0 || t==='⛶ 크게' || t.indexOf('근거 크게')>=0) b.remove(); });
  }
  ...
  /* rpSwitchTab 래핑: 슬림 제목 유지 + 'my' 탭 = AI Agent 렌더 */
  var _rpsw = window.rpSwitchTab;
  window.rpSwitchTab = function(tab){
    if(_rpsw) _rpsw.apply(this, arguments);
    var t=document.getElementById('rpTitle'); if(t) t.textContent='작업 패널';
    if(tab==='my') ensureRpAgents();
  };
  /* rpT 래핑: 닫기/열기 시 재오픈 핸들 동기화 */
  var _rpT = window.rpT;
  window.rpT = function(){ if(_rpT) _rpT.apply(this, arguments); syncReopen(); };

  var rpBusy=false;
  function observeRp(){
    var rp=document.getElementById('rp'); if(!rp){ setTimeout(observeRp, 500); return; }
    try{
      var mo=new MutationObserver(function(){ if(rpBusy) return; rpBusy=true; requestAnimationFrame(function(){ removeEnlarge(); setTimeout(function(){ rpBusy=false; }, 50); }); });
      mo.observe(rp,{childList:true,subtree:true});
    }catch(e){}
  }
  function boot(){ if(!ensureRpAgents()){ setTimeout(boot, 400); return; } removeEnlarge(); observeRp(); ensureReopen(); syncReopen(); }
...
/* ═══════════════════════════════════════════════════════════════
 * §20  ADMIN · AI Agent 관리 탭
 *      · 누구나 올린 임시(PoC) 에이전트를 ADMIN이 최종 승인(공식)/반려/수정/삭제
 *      · 데이터는 §16(window.__agentMkt) 공유, 변경 시 마켓·우측패널 동시 반영
 * ═══════════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  var $  = function(s,r){ return (r||document).querySelector(s); };
  ...
  function areasMeta(){ return window.__agentAreas || []; }
  function areaMeta(id){ var a=areasMeta(); for(var i=0;i<a.length;i++){ if(a[i].id===id) return a[i]; } return {id:id,name:id,icon:'🤖',c:'#4b8ef0'}; }
  function list(){ var st=window.__agentMkt; return (st&&st.list)||[]; }

  function injectStyle(){
    if(document.getElementById('admAgentStyle')) return;
    var css=[
      '#p-agent .aa-stats{display:flex;gap:18px;margin:4px 0 16px;font-size:12px;color:var(--text-3);flex-wrap:wrap}',
      '#p-agent .aa-stats b{font-family:Outfit,sans-serif;color:var(--text-1)}',
      '#p-agent .aa-chips{display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap}',
      '#p-agent .aa-chip{font-size:12px;font-weight:600;padding:7px 14px;border-radius:999px;border:1.5px solid var(--border-2);background:var(--bg-2);color:var(--text-2);cursor:pointer;font-family:inherit}',
      '#p-agent .aa-chip.on{background:var(--accent);color:#fff;border-color:var(--accent)}',
      '#p-agent .aa-row{display:flex;align-items:center;gap:12px;padding:12px 14px;border:1px solid var(--border-1);border-left:3px solid var(--ag-c,var(--accent));border-radius:12px;background:var(--bg-2);margin-bottom:9px}',
      '#p-agent .aa-ic{flex-shrink:0;width:42px;height:42px;border-radius:11px;display:flex;align-items:center;justify-content:center;border:1px solid var(--border-2)}',
      '#p-agent .aa-ic svg{display:block}',
      '#p-agent .aa-main{flex:1;min-width:0}',
      '#p-agent .aa-nm{font-size:13.5px;font-weight:700;color:var(--text-1)}',
      '#p-agent .aa-meta{font-size:11px;color:var(--text-4);margin-top:3px;display:flex;gap:8px;flex-wrap:wrap;align-items:center}',
      '#p-agent .aa-acts{display:flex;gap:6px;flex-shrink:0;flex-wrap:wrap;justify-content:flex-end}',
      '#p-agent .aa-btn{font-size:11.5px;font-weight:700;padding:7px 11px;border-radius:8px;border:1px solid var(--border-2);background:var(--bg-3);color:var(--text-2);cursor:pointer;font-family:inherit;white-space:nowrap}',
      '#p-agent .aa-btn:hover{border-color:var(--accent);color:var(--accent)}',
      '#p-agent .aa-btn.approve{background:#2bb673;border-color:#2bb673;color:#fff}',
  ...
  function ensureAdminAgentTab(){
    var tabs=$('.adm-t'), box=$('.adm');
    if(!tabs || !box) return false;
    injectStyle();
    if(!$('#admAgentTabBtn')){
      var b=document.createElement('button');
      b.className='atb'; b.id='admAgentTabBtn';
      b.setAttribute('onclick',"at(this,'p-agent')");
      b.innerHTML='🤖 AI Agent 관리';
      tabs.appendChild(b);
    }
    if(!$('#p-agent')){
      var p=document.createElement('div'); p.id='p-agent'; p.className='adm-b'; p.style.display='none';
      box.appendChild(p);
    }
    return true;
  }

  window.renderAdminAgents = function(){
    if(!ensureAdminAgentTab()) return;
    var el=$('#p-agent'); if(!el) return;
    var all=list();
    var off=all.filter(function(a){return a.status==='official';}).length;
    var tmp=all.length-off;
    var dl=all.reduce(function(s,a){return s+(a.dl||0);},0);
    var rows=all.filter(function(a){ return fil==='all' || (fil==='official'?a.status==='official':a.status!=='official'); });
    var sb=window.agentStatusBadge||function(){return '';};
    var svg=window.agentAreaSvg||function(){return '🤖';};
    var body=rows.length ? rows.map(function(a){
      ...
      return '<div class="aa-row" style="--ag-c:'+c+'">'
        ...
        + '</div>';
    }).join('') : '<div class="aa-empty">해당 상태의 에이전트가 없습니다.</div>';
    el.innerHTML =
      '<div class="v23-admin-title"><div><div class="v23-title-main">🤖 AI Agent 관리</div>'
      + '<div class="v23-title-sub">누구나 올린 <b>🧪 임시(PoC)</b> 에이전트를 검토해 <b>✅ 공식 승인</b>하거나 반려·수정·삭제합니다. 사용자에게는 <b>공식</b> 에이전트만 다운로드로 제공됩니다.</div></div></div>'
      + '<div class="aa-stats"><span>✅ 공식 <b>'+off+'</b></span><span>🧪 임시(PoC) <b>'+tmp+'</b></span><span>⬇ 총 다운로드 <b>'+dl.toLocaleString()+'</b></span></div>'
      + '<div class="aa-chips">'
        + '<button class="aa-chip'+(fil==='all'?' on':'')+'" onclick="adminAgentFilter(\'all\')">전체 '+all.length+'</button>'
        + '<button class="aa-chip'+(fil==='temp'?' on':'')+'" onclick="adminAgentFilter(\'temp\')">🧪 승인 대기 '+tmp+'</button>'
        + '<button class="aa-chip'+(fil==='official'?' on':'')+'" onclick="adminAgentFilter(\'official\')">✅ 공식 '+off+'</button>'
      + '</div>'
      + body;
  };
  window.adminAgentFilter = function(f){ fil=f; window.renderAdminAgents(); };
  window.adminAgentEdit = function(id){ try{ window.ca && window.ca(); }catch(e){} if(window.openAgentMarket){ window.openAgentMarket(); setTimeout(function(){ window.agentEdit && window.agentEdit(id); }, 360); } else if(window.agentEdit){ window.agentEdit(id); } };

  /* at() 래핑: p-agent 탭 열릴 때 렌더 */
  var _at=window.at;
  if(_at && !window.__admAgentAtWrapped){
    window.__admAgentAtWrapped=true;
    window.at=function(b,id){ if(_at) _at.apply(this,arguments); if(id==='p-agent') window.renderAdminAgents(); };
  }
  /* oa() 래핑: Admin 열릴 때 탭 보장 */
  var _oa=window.oa;
  if(_oa && !window.__admAgentOaWrapped){
    window.__admAgentOaWrapped=true;
    window.oa=function(){ if(_oa) _oa.apply(this,arguments); setTimeout(ensureAdminAgentTab, 60); };
  }

  function boot(){ if(!ensureAdminAgentTab()){ setTimeout(boot, 500); } }
